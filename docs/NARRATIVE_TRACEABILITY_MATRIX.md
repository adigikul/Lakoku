...
| G1-DRIFT | Drift score goal-vs-state; ≥2 → regenerate goal | — | WF step 3 (validate plan) | drift fixture | `review_required_rate` | soak | IN_PROGRESS (`computeDriftScore()` 0–3 atas ActualState; ≥2 → regen goal versi baru. Terbukti di soak. Metrik `review_required_rate` runtime nunggu M6) |
