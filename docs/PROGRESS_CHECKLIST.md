...
**Last updated:** 5 Juli 2026 (PROVIDER LLM NYATA disambung: `lib/ai-gateway/gateway-provider.ts` `createGatewayProvider` pakai AI SDK 7 `generateObject` via Vercel AI Gateway (model string, default `openai/gpt-4.1-mini`, override `NARRATIVE_MODEL`). Aktif via `NARRATIVE_PROVIDER=gateway` di `selectProvider`; default tetap deterministik. Desain aman: LLM hanya menulis prosa pembaca (judul+paragraf), SEMUA metadata terstruktur tetap canon-derived (bungkus provider deterministik) → Layer A/B, reveal/state/thread, & consumer-safe tak bergantung model. `scripts/m6-llm-smoke.ts` + entry `@lakoku/ai-gateway/server`. tsc & gate deterministik hijau (eslint 0, m4 27/27, m5 236/236). CATATAN: live-run terblokir billing (AI Gateway 403 `customer_verification_required` — perlu kartu di akun Vercel). — Sebelumnya: M0 batas paket logis §5.1)
...
| M4 — Template + provider gateway | `[~]` | `lib/ai-gateway/` (schema Zod plan/draft, provider deterministik + `selectProvider()`, gateway consumer-safe, repair loop) + template `lakoku_drama_bangkit_v1`. **Terpasang di jalur runtime nyata** (E2E 21/21). Harness 27/27 hijau. **Provider LLM NYATA `createGatewayProvider` (AI SDK 7 `generateObject` via Vercel AI Gateway) sudah dibuat & di-wire** (`NARRATIVE_PROVIDER=gateway`, model `NARRATIVE_MODEL` default `openai/gpt-4.1-mini`): LLM menulis prosa (judul+paragraf), metadata terstruktur tetap canon-derived → Layer A/B & consumer-safe terjaga. tsc hijau; smoke `scripts/m6-llm-smoke.ts` siap. **Live-run TERBLOKIR billing** (AI Gateway 403 `customer_verification_required`: butuh kartu di akun Vercel). Struktur `packages/ai-gateway` (monorepo) belum |
